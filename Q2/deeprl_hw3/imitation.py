    ...
    for i in range(num_episodes):
        ...
        while t is False:
            if(render):
                env.render()
            s = s.squeeze()
            a = np.argmax(expert.predict(np.expand_dims(s,axis=0)))
            states.append(s)
            actions.append(a)
            s,r,t,debug = env.step(a)

    # print(len(actions))
    # print(actions)
    one_hot_a = np.zeros([len(actions),2])
    one_hot_a[np.arange(len(actions)), actions] = 1
    return np.array(states), one_hot_a
